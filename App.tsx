        ...
        <div className={clsx("flex-1 flex flex-col transition-all duration-300 print:w-full", showAssistant ? "mr-0 md:mr-80 lg:mr-96" : "")}>
          <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm no-print">
            <div className="flex items-center gap-3">
              <button onClick={handleReset} className="bg-gray-100 hover:bg-gray-200 text-gray-600 p-2 rounded-lg transition-colors border border-gray-200" title="Reiniciar">
                  <RotateCcw className="w-5 h-5" />
              </button>
              <div className="bg-blue-600 text-white p-2 rounded-lg"><Settings className="w-5 h-5" /></div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <button onClick={handleExportPDF} disabled={isGeneratingPdf} className="flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  {isGeneratingPdf ? <Loader2 className="w-4 h-4 animate-spin" /> : <Printer className="w-4 h-4" />} {isGeneratingPdf ? '...' : 'PDF'}
              </button>
              <button onClick={handleExportPNG} className="flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm font-medium transition-colors">
                  <Image className="w-4 h-4" /> PNG
              </button>
              <div className="h-6 w-px bg-gray-300 mx-1"></div>
              <button onClick={runAnalysis} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg font-semibold transition-all shadow-sm hover:shadow-md">
                  <Play className="w-4 h-4" /> Calcular
              </button>
              <button onClick={() => setShowAssistant(!showAssistant)} className={clsx("p-2.5 rounded-lg border transition-colors", showAssistant ? "bg-blue-50 border-blue-200 text-blue-600" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50")}>
          ...
          <main className="p-6 max-w-7xl mx-auto w-full grid grid-cols-1 xl:grid-cols-3 gap-6 print:block">
            <div className="xl:col-span-1 space-y-6 no-print">
              <section className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                <h2 className="text-lg font-semibold mb-4 text-gray-800 flex items-center gap-2">Unidades</h2>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1 uppercase">Longitud</label>
                      <select value={state.meta.units.length} onChange={(e) => updateUnit('length', e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                  ...
                  <div>
                      ...
                      <select value={state.meta.units.force} onChange={(e) => updateUnit('force', e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
              ...
              <section className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                <h2 className="text-lg font-semibold mb-4 text-gray-800 flex items-center gap-2">Configuración de Viga</h2>
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-600 mb-1">Longitud Total ({uL})</label>
                    <input type="number" value={state.beam.length} onChange={(e) => updateBeam('length', parseFloat(e.target.value))} className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"/>
                  </div>
                  <div className="flex items-center gap-2 bg-blue-50 p-3 rounded-md">
                      <input type="checkbox" id="useFEM" checked={useFEM} onChange={(e) => setUseFEM(e.target.checked)} className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"/>
                      <label htmlFor="useFEM" className="text-sm text-blue-800 font-medium cursor-pointer">Activar FEM Avanzado</label>
                  </div>
                </div>
              </section>
                ...
                <div className="space-y-3">
                    {state.supports.map((s, idx) => (
                        <div key={s.id} className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg border border-gray-100">
                            ...
                            <select value={s.type} onChange={(e) => updateSupport(s.id, 'type', e.target.value)} className="bg-white border border-gray-300 text-sm rounded px-2 py-1 focus:ring-2 focus:ring-blue-200 outline-none">
                            ...
                            <div className="flex items-center gap-1 flex-1">
                                ...
                                <input type="number" value={s.x} onChange={(e) => updateSupport(s.id, 'x', parseFloat(e.target.value))} className="w-16 text-sm border border-gray-300 rounded px-2 py-1 text-center"/>
...
{/* Results Column */}
          ...
          <footer className="bg-slate-900 text-slate-300 py-8 mt-12 text-center border-t-4 border-blue-500 no-print">
            ...
            <div className="text-sm">
              Creado por{' '}
              <a href="https://www.linkedin.com/in/jaime-m-meneses/" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 font-bold transition-colors ml-1">
                Jaime M. Meneses
              </a>
            </div>
